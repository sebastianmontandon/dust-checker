...
export const FilterBar: React.FC<FilterBarProps> = ({ filters, onChange, onSearch, loading }) => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="flex items-end">
          <button
            onClick={onSearch}
            disabled={loading}
            className={`w-full font-serif tracking-widest font-bold py-2 px-4 rounded transition-all duration-300 
              ${loading 
                ? 'bg-gray-700 text-gray-500 cursor-not-allowed' 
                : 'bg-gradient-to-b from-poe-gold to-poe-goldDim text-black hover:scale-[1.02] hover:brightness-110 shadow-[0_0_15px_rgba(200,170,109,0.3)]'
              }`}
          >
            {loading ? 'ESCANEANDO MERCADO...' : 'BUSCAR OPORTUNIDADES'}
          </button>
        </div>
